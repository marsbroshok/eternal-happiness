...
{
    //set model parameters such as components (faces) and threshold
    //int num_components = 9;
    //double threshold = 10000.0;
    
    self = [self init];
    _model = cv::createEigenFaceRecognizer();//(num_components, threshold);
...
{
    // People table
    const char *peopleSQL = "DROP TABLE IF EXISTS people";
    
    if (sqlite3_exec(_db, peopleSQL, nil, nil, nil) != SQLITE_OK) {
        NSLog(@"The people table could not be deleted.");
    }
    
    // Images table
    const char *imagesSQL = "DROP TABLE IF EXISTS images";
    
    if (sqlite3_exec(_db, imagesSQL, nil, nil, nil) != SQLITE_OK) {
        NSLog(@"The images table could not be deleted.");
    }
}
